...
#include "ui_zgoccontroldialog.h"
...
std::list<std::string> ZGocControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGocControlDialog::listMints;

ZGocControlDialog::ZGocControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGocControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGocControlDialog::~ZGocControlDialog()
{
    delete ui;
}

void ZGocControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGocControlDialog::updateList()
{
...
void ZGocControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGocControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGoc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGocControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGocControlDialog::GetSelectedMints()
{
...
void ZGocControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
