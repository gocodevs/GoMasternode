...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 1 * COIN;
    } else if (nHeight > 201 && nHeight <= 120000) {
        nSubsidy = 35 * COIN;
    } else if (nHeight > 120001 && nHeight <= 670890) {
        nSubsidy = 25 * COIN;
    } else if (nHeight > 670891 && nHeight <= 900790) {
        nSubsidy = 15 * COIN;
    } else if (nHeight > 900791 && nHeight <= 1280990) {
        nSubsidy = 10 * COIN;
    } else if (nHeight > 1280991 && nHeight <= 1487889) {
        nSubsidy = 7 * COIN;
    } else if (nHeight > 1487890 && nHeight <= 1702890) {
        nSubsidy = 5 * COIN;
    } else if (nHeight > 1702891 && nHeight <= 1902839) {
        nSubsidy = 2 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Gocoin
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Gocoin: We use certain sporks during IBD, so check to see if they are
